...
const ListCourses: React.FC<ListCoursesCardProps> = ({ title, description, imageSrc, bgColor, onClick }) => {
  const cardStyle = {
  ...
  return (
    <div className={clsx(styles.activityCard)} style={cardStyle}>
      <div className={styles.content}>
        ...
        <button onClick={onClick} className={styles.button}>Записаться <img src={img} alt="" /></button>
        </div>
      </div>
    </div>
  );
